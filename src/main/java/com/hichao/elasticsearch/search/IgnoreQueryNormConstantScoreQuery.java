...
public class IgnoreQueryNormConstantScoreQuery extends Query {
    protected final Filter filter;
    protected final Query query;

    /** Strips off scores from the passed in Query. The hits will get a constant score
     * dependent on the boost factor of this query. */
    public IgnoreQueryNormConstantScoreQuery(Query query) {
      if (query == null)
     ...
     */
    public IgnoreQueryNormConstantScoreQuery(Filter filter) {
      if (filter == null)
    ...
    protected class ConstantWeight extends Weight {
      ...
      @Override
      public Query getQuery() {
        return IgnoreQueryNormConstantScoreQuery.this;
      }
      ...
      public Explanation explain(AtomicReaderContext context, int doc) throws IOException {
        ...
        if (exists) {
          result.setDescription(IgnoreQueryNormConstantScoreQuery.this.toString() + ", product of:");
          result.setValue(queryWeight);
          result.setMatch(Boolean.TRUE);
          result.addDetail(new Explanation(getBoost(), "boost"));
          result.addDetail(new Explanation(queryNorm, "queryNorm"));
        } else {
          result.setDescription(IgnoreQueryNormConstantScoreQuery.this.toString() + " doesn't match id " + doc);
          result.setValue(0);
    ...
    public Weight createWeight(IndexSearcher searcher) throws IOException {
      return new IgnoreQueryNormConstantScoreQuery.ConstantWeight(searcher);
    }
    ...
    public boolean equals(Object o) {
      ...
      if (!super.equals(o))
        return false;
      if (o instanceof IgnoreQueryNormConstantScoreQuery) {
        final IgnoreQueryNormConstantScoreQuery other = (IgnoreQueryNormConstantScoreQuery) o;
        return 
